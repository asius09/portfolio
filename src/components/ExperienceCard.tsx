...
function formatDateRange(start: string, end?: string) {
  ...
  if (end) {
    formattedEnd =
      end === "Present" ? "Present" : formatDate(end, { showDate: false });
  }
    ...
    <div className={cn("w-full", "mb-2")}>
      ...
      >
        ...
        <span className={cn("flex-1", "min-w-0 text-left", "flex flex-row")}>
          ...
          {company.status === "active" && (
            <span
              className={cn("ml-3", "inline-flex", "items-center")}
      ...
      <div className={cn("w-full", "flex", "items-start", "gap-1")}>
        ...
        <button
          ...
          type="button"
        >
          <span className="flex-start flex flex-col">
            <p
              className={cn(
                "text-base",
                "text-foreground",
            ...
            <span className={cn("text-sm", "text-foreground/60", "text-left")}>
              ...
              {firstPosition.start && (
                <span className="ml-2">
                  {formatDateRange(firstPosition.start, firstPosition.end)}{" "}
                  <span className="text-[10px] font-medium tracking-tight">
                    (
                    {formatDuration(
                      firstPosition.start,
                      firstPosition.end === "Present"
      ...
      {open && (
        <div
          id={`exp-details-${companyName.replace(/\s+/g, "")}`}
          className={cn("pl-6", "pr-2", "pt-1", "pb-1", "animate-fadein")}
        >
          {positions.map((position, idx) => (
            <section key={idx} className="mb-2" aria-label={position.role}>
              {position.bulletPoints.length > 0 && (
                <ul
                  className={cn(
                    "mt-0.5",
                    "ml-4",
                    "marker:text-foreground/50 list-disc",
                    "text-sm",
                    "space-y-2",
                          ...
                          if (linkMatch) {
                            return (
                              <Link
                                ...
                                rel="noopener noreferrer"
                                className={cn(
                                  "font-semibold transition-all duration-200 hover:underline hover:underline-offset-2",
                                )}
                              >
                                {linkMatch[1]}
                              </Link>
